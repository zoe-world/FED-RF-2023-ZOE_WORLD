...
function showTime() {
    ...
    // 3-2. 월 찍기 : getMonth()
    // let monthName = ['1월','2월','3월','4월','5월','6월','7월','8월','9월','10월','11월','12월'];
    // tt[1].innerText = monthName[today.getMonth()];
    tt[1].innerText = today.getMonth() + 1;
    // 찍어보면 기존달보다 1작다
    // 이것은 배열순번에 넣고 찍기 좋도록 (각 나라마다 월을 부르는 이름이 다르니까 배열에 넣으면 꺼내쓰기 좋으니까)
    // 월순번이 리턴되서 배열은 0부터 시작하니까
    // => 따라서 숫자월을 찍고싶으면 +1함
    ...
    // 테스트용 한자리 숫자 임의로 넣어보기
    // H = 22;
    tt[5].innerText = addZero(H > 12 ? H - 12 : H);
    // 12보다 크면 12를 빼주고 한자리 일때는 0처리는 항상해줌!
...
// rdm = rdm * 7;
// console.log('rdm * 7:',rdm)
// console.log('rdm * 7 내림:',Math.floor(rdm))
// console.log('rdm * 7 올림:',Math.ceil(rdm))
...
// 직전랜덤수 : 초기셋팅은 0~3 사이수가 아닌 수
let bNum = 1000;

function colorImg() {
    // 1. 난수 만들기 : 0~3 -> 1~4 를 만들고 내림처리
    let rdmNum = Math.floor(Math.random() * 4);
    console.log("랜덤수:", rdmNum);
    
    // 2. 직전 랜덤수가 같을 경우 다시 난수발생하기
    // while(조건true){코드} 사용하기!
    // 항상 직전랜덤수는 전역변수로 저장하고 매번 비교한다!!
    while(rdmNum==bNum){ //현재랜덤수가 직접 랜덤수와 같냐?
        // 같으면 다시 랜덤발생
    ...
    target.forEach((ele, idx) => {
        if (idx == rdmNum) ele.classList.add("on");
