...
function bindMod() {
    ...
    // 2. 선택박스 초기화(매번업데이트 됄 때를 대비하여)
    modSel.innerHTML = `
        <option value="show">항목선택</option>
    `;

    // 3. idx 로 value값을 만들고 제목으로 항목명을 만들기!
    orgData.forEach((v) => {
        modSel.innerHTML += `
...
function modifyData() {
    ...
    // 현재 선택된 수정선택박스의 value 값
    let selIdx = modSel.value;
    console.log("수정할꼬얌~!", selIdx);
    ...
    orgData.find((v) => {
        if (v.idx == selIdx){ 
            // 고유 idx값인 경우 입력값으로 업데이트하기
            // 3. 선택 배열값이 객체의 제목과 내용을 다시 넣고 업데이트하기!!
            v.tit = modTit.value;
