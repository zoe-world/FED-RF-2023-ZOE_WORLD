...
/******************************************************* 
    ...
    1. 사용 이벤트: scroll
    -> 스크롤 바가 있는 페이지에서 또는 부분 박스에서 
    스크롤 바가 이동할 때 발생하는 이벤트!
    -> 주의! wheel 이벤트와는 다르다! 무엇이?
    스크롤바가 이동하지 않아도 마우스 휠이 작동될 때 발생한다!
    (휠 이벤트는 모바일에서 사용불가!)
    ...
    -> 보이는 화면 상단을 기준으로 이것보다 위로 갈경우
    마이너스값을 리턴한다!
    -> 기준: 보이는 화면크기를 기준하면 된다!
    -> 윈도우 화면 전체: window.innerHeight
    예) 화면의 3분의2는 window.innerHeight/3*2
    예) 화면의 4분의3는 window.innerHeight/4*3
...
function showIt() {
  ...
  // if(scTop > pos1) scAct[0].classList.add('on');
...
// 스크롤한계값 : 전체 document 높이 - 화면 높이
let scLimit = docH - winH;

console.log("스크롤한계값:", scLimit ,winH);

// 비례식 -->> 스크롤한계값 : 윈도우높이 = 스크롤이동값 : 이미지 이동값
// 내가 구하고 싶은 건 이미지 이동값 이므로
