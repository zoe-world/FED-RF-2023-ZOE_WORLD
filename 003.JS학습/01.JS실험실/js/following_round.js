...
function loadFn(){
    ...
    for(let i=1; i<=50; i++){
        hcode += `
        <div>
            <img src="../images/dress/${i}.jpg" alt="">
    ...
    */
    ...
    // 선택요소의 크기 JS
    // width 는 clientWidth, height 는 clientHeight
    console.log('무버width:',gap);

    // 2. 이벤트대상에 마우스 무브 이벤트가 적용될 때
    // 무버가 따라다니게 기능 구현
    myBody.onmousemove = e => { // e- 이벤트 전달변수
        // 1. 위치값 가져오기(박스중앙위치로 고정!)
        let posx = e.pageX - gap;
        let posy = e.pageY - gap;
        // let posy = e.clientY - gap;
        // -> 만약 .mover가 postion:fixed 면 
        // 브라우저 화면에서의 위치인 clientY를 사용해야함!
