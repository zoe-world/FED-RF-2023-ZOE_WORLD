...
// 1-2. 아이콘박스
const iconBox = dFn.qsa(".icon");

// console.log(txtBox,iconBox);
...
function scrollFn() {
  ...
  // 1. 대상2: 아이콘박스 패럴렉스 호출!
  iconBox.forEach((ele) => moveEl(dFn.getBCR(ele), ele, setH1));
} /////////// scrollFn 함수 /////////////
...
function moveEl(elPos, ele, setH) {
  ...
  // (1) elPos - 위치값(getBCL값)
  // (2) ele - 대상요소(패럴렉스대상)
  // (3) setH - 움직일 범위수(클수록 빨리 움직인다)

//   console.log("위치:", elPos, "\n대상:", ele, "\n범위:", setH);

  // [패럴렉스 범위 : 위도우 높이값 ~ 0]
  // 화면에서 완전히 사라질때 범위 0보다 작다(약간의 마이너스값)
