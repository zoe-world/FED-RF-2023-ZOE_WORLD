 ...
 * -> 이때부터 생성자 함수로 인정받고, 사용한다는 것을 의미
 * 3. 만약 생성자 함수의 속성/메서드를
 * 인스터스 코딩구역에서 활용하고자, 할 경우 
 * this키워드로 생성자함수 멤버등록을 하여 사용함!
 * -> 인스턴스 코딩구역에서 선언이 가능해짐
 * 변수에 담아 하위 속성/메서드로 호출이 가능해짐!
 * 4. 유의사항 : 생성자함수 내부에서 this키워드로
 * 등록된 속성/메서드는 내부에서 호출시에도 반드시
 * this키워드를 사용하여 호출해야함(안쓰면 에러남)
...
function MakeDallyeok(selEl){ 
    ...
    // (2) 오늘날짜 객체
    const today = new Date();    // (3) 년도요소 : .yearTit
