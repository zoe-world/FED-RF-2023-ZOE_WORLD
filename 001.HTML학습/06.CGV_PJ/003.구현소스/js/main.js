...
pmenu.forEach((ele) => {
  domFn.addEvt(ele, "click", (e) => {
    ...
    // 호출확인!
    console.log("나야냐", me, mtit, myCode[mtit]);

    // 2. 스크린박스에 iframe src 재구성하여 넣기
    screen.innerHTML = `
            <iframe src="https://www.youtube.com/embed/${myCode[mtit]}?autoplay=1" allow="autoplay"></iframe>
        `;

    // 3. 추가구현: 클릭된 a요소의 부모인 li에 클래스 on 추가해주기
    // - on을 주면 미리 세팅된대로 li가 일어나있다
    // 부모로 올라가기 ele.parentElement

    // 3-1. 클래스 on 지우기 초기화 : 대상 mlist 변수
