...
function MainComponent() {
  ...
  // const [변수명, set첫대변수명] = React.useState(초기값)
  // dataNum은 데이터를 구분하는 번호저장 후크변수다!
  // 데이터 구분값으로 배열순번을 생각하여 처음에 로딩될 
  // 데이터가 0번째 즉. 첫번째 배열순번 데이터를 값으로 셋팅함!
  // 순번값을 셋팅함!
  ...
  // 순수 useEffect
  React.useEffect(()=>{
    console.log('useEffect순수버전')
    // console.log('useEffect순수 구역 제이쿼리',$('.img-box'))
    // $('.btn-gong').hide()
  });
  ...
  return (
    <React.Fragment>
      ...
      {/* 2. 내용박스 */}
      <section>
        <h2>{dataNum?'효진은 오늘도 간지납니다!':'공유는 오늘도 멋찝니다!'}</h2>
        {/* 이미지 */}
        <div className="img-box">
          <img src={dataNum?'./images/vans/hyojin.jpg':'./images/vans/gongyoo.jpg'} alt={dataNum?'멋진효진':'멋진공유'} />
        </div>
      </section>
      {/* 3. 데이터 변경 버튼 */}
...
// 서브 컴포넌트 (자식컴포넌트는 부모컴포넌트로부터 데이터를 props속성을 통하여 전달 받는다.)
// 상품 html코드 구성 컴포넌트 ///////////
