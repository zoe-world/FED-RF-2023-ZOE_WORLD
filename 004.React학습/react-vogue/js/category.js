...
function MainComponent() {
  ...
  const chgCat = (val) => {
    ...
    // 새로고침 등 페이지 리로드시 처음들어온
    // 파라미터로 페이지가 변경되며 새로고쳐짐
    // 이유는 처음들어온 cat=카테고리명 때문임!
    // 그래서 리액트컴포넌트 업데이트 시, 
    // 상단 url 마지막 cat 키값을 실제 카테고리명으로 업데이트 해준다.
    // => history 객체는 window하위 객체임
  ...
  return (
    ...
    // 순서:
    // 1. creatContext()를 생성하여 사용할 곳에 import한다!
    // 2. 최상위 컴포넌트에서 컨텍스트 프로바이더를 셋팅한다
    // 3. value 속성에 공유할 변수/함수를 넣어준다!
    ...
    /* 
      <생성컨텍스트명.Provider value={보낼 함수명을 보냄}>
        <하위 컴포넌트들 />
        <하위 컴포넌트들 />
      </생성컨텍스트명.Provider/>
    */
