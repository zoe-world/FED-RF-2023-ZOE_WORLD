...
$(() => {
  ...
  /****************************************** 
          ...
          원리: 마우스 포인터 위치 중 x축 값만
          이용하여 처음 찍은 위치와 드래그 하여 
          마지막에 찍은 위치를 비교하여 방향을 결정한 후 
          이전/다음 이미지로 순서대로 넘겨서
          자동차를 보여준다!
      ******************************************/
  ...
  // 1. 변수 셋팅하기
  // (1) 드래그 상태변수 : 0 - 드래그 아님 / 1 - 드래그 중
  let drag = 0;

  // (2) 클릭시 위치변수(드래그시작점 - 실제 할당값)
  let point = 0;
...
// 마우스가 나갈떄도, 해체처리해야 드래그 하다가 나갈때 괜찮음
...
const rotateCar = dir => {
    ...
    if(dir){
        fnum++;
        if(fnum == 50) fnum = 0;
        // 마지막순번은 49번 이므로, 50번에서 처음순번인 0으로 돌아가게 해줌
    } /////// if /////////
    // 2. 이미지 순번 감소처리
    else{
        fnum--;
        if(fnum == -1) fnum = 49;
        // 첫순번은 0이므로, -1이 되면, 마지막 순번인 49으로 돌아가게 해줌!
    }////////// else //////////
   ...
   console.log('순번',fnum);
    ...
    // 선택요소:visible 은 display:none이 아닌 요소 선택함!
